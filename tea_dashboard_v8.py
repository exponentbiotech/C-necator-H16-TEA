#!/usr/bin/env python3
"""
Leatherback Fairfield TEA Dashboard — v8
========================================
...
Scope:
  ...
  - CapEx sliders: acquisition cost + added major CapEx + (fed-batch only)
    OTR-retrofit CapEx for brewery-vessel impeller/sparger upgrade
...
Revision history:
  ...
  v8 — Adds a fed-batch operating mode as a first-class alternative to the
       ...
       (glucose fed-batch, 121 g/L, 76 % PHB), Ryu 1997 (phosphate-
       limited fed-batch, 281 g/L, 67 % PHB), and Budde 2011 (engineered
       R. eutropha fed-batch scale-up in commercial vessels). The SCP
       revenue math is unchanged in v8 fed-batch mode; the reduced crude-
       protein content of N-limited residual biomass is flagged as a
       caveat in the app captions and accompanying memo, and is not
       yet auto-derated in the revenue line.

Run locally:
    cd ~/Downloads
    streamlit run tea_dashboard_v8.py
...
def scenario_reference_ids(model_key: str, r: Any, assumption_kwargs: Dict[str, Any], defaults: Any) -> List[str]:
    ...
    if model_key in {"pha", "bio"}:
        ids.extend(["kim_1994", "ryu_1997", "budde_2011", "plasmid_2025"])
...
st.title("Leatherback Fairfield TEA Dashboard v8")
st.caption(
    "Dedicated Fairfield dashboard | AB InBev Fairfield brewery | "
    "Continuous OR fed-batch mode toggle | "
    "Scenarios 1 and 2 only | NCIMB 11599 | "
    "Continuous base: 60 g/L, 24 h HRT, 85% uptime | "
    "Fed-batch base: 150 g/L, 68% PHB, 76 h cycle, 68% duty cycle (Kim 1994, Ryu 1997, Budde 2011)"
)
