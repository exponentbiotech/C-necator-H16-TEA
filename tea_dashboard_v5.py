#!/usr/bin/env python3
"""
Leatherback Fairfield TEA Dashboard — v5
========================================
...
st.set_page_config(
    page_title="Leatherback Fairfield TEA Dashboard v5",
    page_icon="🧬",
    layout="wide",
...
def _render_chat_text(text: str, *, trailing_cursor: bool = False) -> str:
    """Render chat text as plain HTML so $ and formulas don't get mangled by markdown/math parsing."""
    safe = html.escape(text)
    if trailing_cursor:
        safe += ' <span style="opacity:0.6;">|</span>'
    return (
...
def render_chat(model_key: str, assumption_kwargs: Dict[str, Any],
    ...
    for msg in st.session_state.chat_messages:
        with st.chat_message(msg["role"]):
            st.markdown(_render_chat_text(msg["content"]), unsafe_allow_html=True)

    if user_input := st.chat_input("Ask about the TEA results, assumptions, or references..."):
        st.session_state.chat_messages.append({"role": "user", "content": user_input})
        with st.chat_message("user"):
            st.markdown(_render_chat_text(user_input), unsafe_allow_html=True)
...
scp_target_price = st.sidebar.slider("SCP selling price ($/kg)", 0.10, 3.00, SCP_TARGET_PRICE, 0.01, key="v5_scp_price")
...
st.caption("Leatherback Fairfield TEA Dashboard v5 — dedicated Fairfield-only dashboard built on the current TEA workflow, with fixed phase volumes and scenario-locked assumptions.")
