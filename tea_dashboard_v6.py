#!/usr/bin/env python3
"""
Leatherback Fairfield TEA Dashboard — v6
========================================
...
v6 changes vs. v5 (Apr 15 2026):
  ...
  - UI now clearly distinguishes SCP "market price" (revenue input) from
    SCP MSP (cost-floor output), and carries reference links.
...
Scope:
  ...
  - Scenarios modeled: S1 (Jelly Belly COD), S2 (70/30 JB COD + DLP),
    S3_HGP (JB COD → human-grade protein)
  - Finance: 9 % discount rate, 10-year horizon
  - CapEx sliders: acquisition cost + added major CapEx

Run locally:
    cd ~/Downloads
    streamlit run tea_dashboard_v6.py
...
st.set_page_config(
    page_title="Leatherback Fairfield TEA Dashboard v6",
    page_icon="🧬",
    layout="wide",
...
def _require_app_password() -> None:
    ...
    st.title("Leatherback Fairfield TEA Dashboard v5")
...
# SCP / HGP market price defaults (v6 — see module docstring for rationale)
# Feed-grade SCP: bracketed by fishmeal ($1.45–1.80/kg, FRED PFISHUSDM 2025),
# Calysta FeedKind target ($1.50–2.00/kg, Rabobank/Aquafeed.com 2025), and
# Unibio UniProtein (~$2.00/kg). Central value $1.80/kg.
SCP_TARGET_PRICE = 1.80              # feed-grade SCP selling price ($/kg)
SCP_CREDIT_PRICE = 2.00              # SCP co-product credit for PHA MSP calc
# Human-grade protein: bracketed by mycoprotein ingredient ($3–5/kg), pea
# protein isolate ($3–8/kg), and Solar Foods Solein target ($5–10/kg).
# Central value $5.00/kg.
HGP_TARGET_PRICE = 5.00              # human-grade protein selling price ($/kg)
HGP_DSP_PREMIUM_PER_KG_CDW = 0.80    # food-grade purification premium ($/kg CDW)
HGP_SCP_RECOVERY = 0.70              # lower recovery vs feed (0.85) due to food-grade DSP
FEED_SCP_RECOVERY = 0.85
SCP_DRY_FACTOR = 0.92                # shared: 8 % moisture final product
...
class FairfieldScenario:
    ...
    notes: str
    # v6 additions — product grade + recovery defaults for feed vs human-grade
    product_grade: str = "feed"                     # "feed" or "human"
    scp_recovery_frac: float = FEED_SCP_RECOVERY    # 0.85 feed / 0.70 HGP
    scp_dry_factor: float = SCP_DRY_FACTOR          # 0.92 shared
    extra_dsp_cost_per_kg_cdw: float = 0.0          # HGP adds $0.80/kg CDW
    default_product_price: float = SCP_TARGET_PRICE # $1.80 feed / $5.00 HGP
...
FAIRFIELD_SCENARIOS: Dict[str, FairfieldScenario] = {
    ...
    "S3_HGP": FairfieldScenario(
        ...
        scp_dry_factor=SCP_DRY_FACTOR,
        extra_dsp_cost_per_kg_cdw=HGP_DSP_PREMIUM_PER_KG_CDW,
        default_product_price=HGP_TARGET_PRICE,
    ),
}


# v6: plot colors per scenario (used by multi-scenario figures)
SCENARIO_PLOT_COLORS: Dict[str, str] = {
    "S1": "#0ea5e9",
    "S2": "#8b5cf6",
    "S3_HGP": "#22c55e",
}
...
def _sellable_scp_kg(
    annual_cdw_kg: float,
    phb_content_frac: float,
    recovery_frac: float = FEED_SCP_RECOVERY,
    dry_factor: float = SCP_DRY_FACTOR,
) -> float:
    """Sellable SCP / HGP in kg. Recovery defaults to feed-grade 0.85; HGP uses 0.70."""
    return annual_cdw_kg * (1.0 - phb_content_frac) * recovery_frac * dry_factor
...
def _fairfield_single_result(
    ...
    # v6: price defaults per scenario product grade (feed $1.80 / HGP $5.00).
    scp_target_price = float(overrides.get("scp_target_price", scenario.default_product_price))
    scp_credit_price = float(overrides.get("scp_credit_price", SCP_CREDIT_PRICE))
    # v6: downstream recovery & food-grade DSP premium are scenario-level for HGP.
    scp_recovery_frac = float(overrides.get("scp_recovery_frac", scenario.scp_recovery_frac))
    scp_dry_factor = float(overrides.get("scp_dry_factor", scenario.scp_dry_factor))
    extra_dsp_cost_per_kg_cdw = float(
        overrides.get("extra_dsp_cost_per_kg_cdw", scenario.extra_dsp_cost_per_kg_cdw)
    )
    ...
    annual_scp_kg = _sellable_scp_kg(annual_cdw_kg, phb_content_frac, scp_recovery_frac, scp_dry_factor)
    ...
    downstream_cost = annual_cdw_kg * (downstream_cost_per_kg_cdw + extra_dsp_cost_per_kg_cdw)
...
def fig_v5_outputs(results: List[FairfieldResult]) -> plt.Figure:
    scenario_ids = list(SCENARIO_PLOT_COLORS.keys())
    n = len(scenario_ids)
    fig, axes = plt.subplots(1, n, figsize=(6.0 + 4.5 * (n - 1), 6.2), constrained_layout=True)
    if n == 1:
        axes = [axes]
    ...
    for ax, scenario_id in zip(axes, scenario_ids):
        ...
        scp_label = "HGP" if scenario.product_grade == "human" else "SCP"
        ax.bar(x, pha, width=0.58, color="#0ea5e9", label="PHA", edgecolor="white", linewidth=0.8)
        ax.bar(x, scp, width=0.58, bottom=pha, color="#22c55e", label=scp_label, edgecolor="white", linewidth=0.8)
        ax.set_xticks(x)
...
def fig_v5_msp_and_cash(results: List[FairfieldResult]) -> plt.Figure:
    ...
    # PHA MSP only makes sense for scenarios that actually produce PHA (phb > 0).
    for scenario_id, color in SCENARIO_PLOT_COLORS.items():
        scenario = FAIRFIELD_SCENARIOS[scenario_id]
        ...
        if scenario.phb_content_frac > 0.0:
    ...
    for ax in [ax1, ax2]:
        ...
        ax.grid(alpha=0.25)
        ax.legend(fontsize=9)
    ax1.set_title("PHA MSP with SCP credit (PHA-producing scenarios only)")
...
def fig_v5_npv_vs_price(
    ...
    # PHA-price sweep only applies to scenarios that produce PHA.
    for scenario_id, color in SCENARIO_PLOT_COLORS.items():
        scenario = FAIRFIELD_SCENARIOS[scenario_id]
        if scenario.phb_content_frac <= 0.0:
            continue
...
def fig_v5_feedstock_sensitivity(results: List[FairfieldResult], phase: str, scp_credit_price: float) -> plt.Figure:
    ...
    w = 0.32
    pha_scenarios = [(sid, col) for sid, col in SCENARIO_PLOT_COLORS.items()
                     if FAIRFIELD_SCENARIOS[sid].phb_content_frac > 0.0]
    for idx, (scenario_id, color) in enumerate(pha_scenarios):
        r = _fairfield_result(results, phase, scenario_id)
        vals = []
        for f in factors:
        ...
        offset = (idx - (len(pha_scenarios) - 1) / 2.0) * w
        ax.bar(x + offset, vals, width=w, color=color, edgecolor="white", linewidth=0.8, label=scenario_id)
    ax.set_xticks(x)
...
st.sidebar.caption(
    "**Market price** (revenue input) ≠ **MSP** (cost floor output). "
    "Feed-grade SCP benchmarks: fishmeal $1.45–1.80/kg (FRED PFISHUSDM 2025), "
    "Calysta FeedKind $1.50–2.00/kg (Aquafeed.com 2025). HGP benchmarks: "
    "mycoprotein $3–5/kg, Solar Foods Solein target $5–10/kg."
)
scp_target_price = st.sidebar.slider(
    "Feed-grade SCP market price ($/kg)", 0.30, 8.00, SCP_TARGET_PRICE, 0.05, key="v6_scp_price",
    help="Applied to S1 and S2 (feed SCP). Default $1.80/kg = midpoint of fishmeal / Calysta / UniProtein market range."
)
hgp_target_price = st.sidebar.slider(
    "Human-grade protein market price ($/kg)", 2.00, 15.00, HGP_TARGET_PRICE, 0.10, key="v6_hgp_price",
    help="Applied only to S3_HGP. Default $5.00/kg sits between mycoprotein ingredient pricing and Solein target pricing."
)
scp_credit_price = st.sidebar.slider(
    "SCP credit price for PHA MSP ($/kg)", 0.10, 5.00, SCP_CREDIT_PRICE, 0.05, key="v5_scp_credit",
    help="Used only in the PHA-MSP-with-credit calculation; not revenue."
)
...
scenario_overrides = {
    "S1": {
        ...
        "scp_target_price": scp_target_price,          # feed-grade
    },
    "S2": {
        "titer_gL": s2_titer,
    ...
    "S3_HGP": {
        # HGP uses S1 fermentation assumptions with zero-PHB and food-grade DSP.
        "jb_sugar_price": jb_sugar_price,
        "jb_pretreat_cost": jb_pretreat_cost,
        "scp_target_price": hgp_target_price,          # human-grade
    },
}
...
common_overrides = {
    ...
    # scp_target_price deliberately NOT in common_overrides — each scenario sets its own
    # (feed-grade or human-grade), so feed and HGP sliders apply to the right scenarios.
    ...
    with c2:
        st.markdown(
            (
                ...
                f"<div style='margin-top:6px;opacity:0.8;font-size:0.9em;'>Market price = revenue input. MSP = cost-floor output. These are different things and should not be conflated.</div>"
                f"</div>"
            ),
            unsafe_allow_html=True,
        )
...
sidebar_snapshot = {
    ...
    "scp_target_price_feed": scp_target_price,
    "hgp_target_price": hgp_target_price,
    "scp_credit_price": scp_credit_price,
    "discount_rate": discount_rate,
    "npv_years": npv_years,
}
...
st.caption(
    "Leatherback Fairfield TEA Dashboard v6 — dedicated Fairfield-only dashboard built on the current TEA workflow, "
    "with fixed phase volumes and scenario-locked assumptions. v6 corrects the SCP market price and adds scenario S3_HGP "
    "for human-grade protein."
)
